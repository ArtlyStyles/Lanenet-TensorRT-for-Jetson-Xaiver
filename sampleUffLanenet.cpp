...
const char* OUTPUT_BLOB_NAME0 = "lanenet_model/vgg_backend/instance_seg/pix_embedding_conv/Conv2D";
...
void doInference(IExecutionContext& context, float* inputData, float* detectionOut, int* keepCount, int batchSize)
{
    ...
    CHECK(cudaMemcpyAsync(detectionOut, buffers[outputIndex0], batchSize * INPUT_C * INPUT_H * INPUT_W * sizeof(float), cudaMemcpyDeviceToHost, stream));
...
int main(int argc, char* argv[])
{
    ...
    auto fileName = locateFile("lanenet_instance_model.uff");
    ...
    int OUTPUT_C = INPUT_C + 1;
    
    // Host memory for outputs.
    //vector<float> detectionOut(N * detectionOutputParam.keepTopK * 7);
    vector<float> detectionOut(N * OUTPUT_C * INPUT_H * INPUT_W);
    ...
    for (int i = 0, volImg = OUTPUT_C * INPUT_H * INPUT_W; i < N; ++i)
    {
        for (int c = 0; c < INPUT_C; ++c)
        {
	    ...
	    gLogInfo << std::endl;
            ...
            for (unsigned j = 0, volChl = INPUT_H * INPUT_W; j < volChl; ++j)
            {
	         ppms[i].buffer[j * INPUT_C + INPUT_C - 1 - c] = (unsigned char)((detectionOut[i * volImg + j * OUTPUT_C + c] - dmin) * 255 / range + 0.5);
            }
        }
